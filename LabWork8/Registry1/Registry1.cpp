...
void ExportRegistryKey(HKEY hKey, const std::wstring& keyName, const std::wstring& outputPath) {
    ...
    WCHAR subKeyName[bufferSize];
    DWORD subKeyNameSize = bufferSize;
    for (DWORD i = 0; RegEnumKeyEx(hSubKey, i, subKeyName, &subKeyNameSize, nullptr, nullptr, nullptr, nullptr) == ERROR_SUCCESS; ++i) {
        std::wstring subKeyPath = keyName + L"\\" + subKeyName;
        ExportRegistryKey(hKey, subKeyPath, outputPath);
        subKeyNameSize = bufferSize;
    }
